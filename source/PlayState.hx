...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'lost-state':
				dialogue = CoolUtil.coolTextFile(Paths.txt('lost-state/keiDialogue'));
				trace(dialogue);
			case 'maybe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('maybe/keiDialogue'));
				trace(dialogue);
			case 'awareness':
				dialogue = CoolUtil.coolTextFile(Paths.txt('awareness/keiDialogue'));
				trace(dialogue);
			case 'senpai':
		...
		switch(stageCheck)
		{
			...
			case 'day':
			{
					...
					defaultCamZoom = 0.8;
					curStage = 'day'; 
					var bg:FlxSprite = new FlxSprite(-800, -490).loadGraphic(Paths.image('day/bg_day'));
					bg.antialiasing = true;
			...
			case 'even':
			{
					...
					defaultCamZoom = 0.8;
					curStage = 'even'; 
					var bg:FlxSprite = new FlxSprite(-800, -490).loadGraphic(Paths.image('even/bg_evn'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 1.9));
			...
			case 'nght':
			{
					...
					defaultCamZoom = 0.8;
					curStage = 'nght'; 
					var bg:FlxSprite = new FlxSprite(-800, -490).loadGraphic(Paths.image('nght/bg_night'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 1.9));
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == 'awareness')
					{
					   FlxG.switchState(new VideoState('assets/videos/ending.webm', new StoryMenuState()));
